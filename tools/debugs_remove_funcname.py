...
funcname_re = re.compile(
    r"""
    (?P<pre>(z?log_debug|vnc_zlog_debug_verbose|vnc_zlog_debug_any|grpc_debug|zfpm_debug|PCEP_DEBUG(_PATH)?|LOG_GR|PATH_TED_DEBUG)\s*\([\s\n]*")
    :?\s*(%s|\(%s\)|\[%s\])[:, \t]*
    (?P<post>.*")
...
funcname_hdr_re = re.compile(
    ...
    (?P<hdr>EXT|\[BGP_GR\]|MPLS-TE|ISIS-TE|RI|SR)\s*(%s|\(%s\)|\[%s\])
    (?P<post>.*")
    \s*,\s*(__func__|__PRETTY_FUNCTION__)\s*
    """,
    re.X,
